...
abstract class AbstractGraphTests {
    ...
    fun findEulerLoop(findEulerLoop: Graph.() -> List<Graph.Edge>) {
        ...
        val oneGraph = GraphBuilder().apply {
            val a = addVertex("A")
        }.build()
        val oneLoop = oneGraph.findEulerLoop()
        assertTrue(oneLoop.isEmpty(), "Euler loop should be empty for the graph with 1 vertex")

        val notEdgesGraph = GraphBuilder().apply {
            val a = addVertex("A")
            val b = addVertex("B")
            val c = addVertex("C")
            val d = addVertex("D")
        }.build()
        val notEdgesLoop = notEdgesGraph.findEulerLoop()
        assertTrue(notEdgesLoop.isEmpty(), "Euler loop should be empty for the graph without edges")

        val withPathGraph = GraphBuilder().apply {
            val a = addVertex("A")
            val b = addVertex("B")
            addConnection(a, b)
        }.build()
        assertEquals(emptyList(), withPathGraph.findEulerLoop(), "есть путь но нет цикла")
